...
public class MealAnalysisCacheService {
     ...
     * across all users.
     */
    public String extractionKey(java.util.List<byte[]> images, String description) {
        StringBuilder imageDigest = new StringBuilder();
        if (images == null || images.isEmpty()) {
            imageDigest.append("no-image");
        } else {
            // Order-sensitive by design: the same photos in a different order are a
            // different request as far as the vision model is concerned.
            for (byte[] bytes : images) {
                if (bytes == null || bytes.length == 0) continue;
                imageDigest.append(hash("img", bytes)).append(FIELD_SEPARATOR);
            }
        }
        return hash(KIND_EXTRACTION, imageDigest.toString(),
