 ...
 *
 *   Stage 1 — Vision/Extraction  (multimodal model: Claude / Gemini fallback)
 ...
 *
 *   Stage 2 — Clinical Reasoning (text-only model: Claude / Gemini fallback)
    ...
    public String processMealAnalysis(byte[] imageBytes, String prompt) {
        try {
            ...
            if (status == 503 || status == 529) {
                log.warn("[MealAnalysis] Claude busy or unavailable ({}). Switching to fallback provider: Gemini.", status);
                return fallbackProvider.analyzeFoodImage(imageBytes, prompt);
            }
            throw e;
        }
